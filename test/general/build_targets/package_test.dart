...
''');
  ...
  group('.NET TPK', () {
    testUsingContext('Release build succeeds', () async {
    ...
    testUsingContext(
      ...
      expect(
        logger.statusText,
        contains('The TPK was signed with a default certificate.'),
      );
    }, overrides: <Type, Generator>{
      FileSystem: () => fileSystem,
      ProcessManager: () => processManager,
...
''');
    ...
    testUsingContext('Build fails if no security profile is found', () async {
      ...
      await expectLater(
        ...
        throwsToolExit(
            message: 'Native TPKs cannot be built without valid certificates.'),
      );
    }, overrides: <Type, Generator>{
      FileSystem: () => fileSystem,
      ProcessManager: () => processManager,
...
void _installFakeEngineArtifacts(Directory engineArtifactDir) {
  for (final String dir in <String>[
    ...
    engineArtifactDir.childDirectory(dir).createSync(recursive: true);
  }
